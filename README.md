...
A custom client must be built with `followRedirects(false)`; configuration validation
rejects it otherwise. The token request carries the client secret (or assertion) in its
form body, which OkHttp would replay to another host on a 307/308 redirect.
